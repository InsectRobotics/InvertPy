...
def hebbian(w, r_in, r_out, rein, learning_rate=1., w_rest=1.):
    # When KC > 0 and MBON > 0 increase the weight (if KC < 0 it is reversed)
    # When KC > 0 and MBON = 0 decrease the weight (if KC < 0 it is reversed)
    # When KC = 0 no learning happens
    rein = rein[np.newaxis, ...]
    r_in = r_in[..., np.newaxis]
    r_out = r_out[np.newaxis, ...]
    return w + learning_rate * (rein * np.outer(r_in, r_out) - w + w_rest)
